...
async function loadAdminPage(token) {
  ...
  function renderTable(products) {
    ...
    products.forEach(p => {
      ...
      const visibleIcon  = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>`;
      const hiddenIcon   = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>`;
      ...
      tr.innerHTML = `
        ...
        <td>${formatPrice(p.listed_price_flat)}</td>
        <td>${statusCell}</td>
        <td>
          <div class="table-actions">
            <button class="btn-visibility ${p.is_visible ? 'btn-visible' : 'btn-hidden'}" data-id="${p.id}" data-visible="${p.is_visible}" title="${p.is_visible ? 'Hide from store' : 'Show in store'}">
              ${p.is_visible ? visibleIcon : hiddenIcon}
            </button>
      ...
      tr.querySelector('.btn-visibility').addEventListener('click', async btn => {
        const el      = btn.currentTarget;
        const id      = parseInt(el.dataset.id, 10);
        const makeVisible = el.dataset.visible === 'false';
        el.disabled   = true;
        try {
          await apiToggleVisibility(token, id, makeVisible);
          await reloadTable();
        } catch (err) {
          if (err.status === 401) { handleAuthError(); return; }
